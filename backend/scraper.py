...
def search_unsplash_image(query):
    """Searches for a real image on Unsplash as a high-quality fallback."""
    try:
        # Use Bing to search specifically on Unsplash for better results without an API key
        search_query = f"{query} site:unsplash.com"
        print(f"Buscando imagen real en Unsplash para: {search_query}")
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"}
        url = f"https://www.bing.com/images/search?q={urllib.parse.quote(search_query)}"
        response = requests.get(url, headers=headers, timeout=10)
        soup = BeautifulSoup(response.text, "html.parser")
        ...
        for a in soup.find_all("a", class_="iusc"):
            ...
            if m:
                data = json.loads(m)
                img_url = data.get("murl")
                # Ensure it's a direct Unsplash image link or at least a high-res version
                if img_url and "unsplash.com" in img_url and is_valid_image(img_url):
                    # Unsplash images usually have a format we can optimize
                    if "?" in img_url:
                        img_url = img_url.split("?")[0]
                    return f"{img_url}?q=80&w=1024"
        return ""
    except Exception as e:
        print(f"Error buscando imagen en Unsplash: {e}")
        return ""
